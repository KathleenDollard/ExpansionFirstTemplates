...
namespace ExpansionFirst.Common
{
   public class MetadataContextStack : IHasLookupValue
   {
      ...
      public Tuple<string, T> RecentOfType<T>()
      {
         // walk up the stack
         foreach (var current in metadataStack)
         {
            var firstMatch = current
                           .GetMatching(x => x.Value is T)
                           .Select(x=>Tuple.Create(x.Name,(T) x.Value))
                           .FirstOrDefault();
